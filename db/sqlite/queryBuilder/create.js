...
const setTimestamps = (model, sqlite) => {
    const { attributes, timestamps, localtime, paranoid } = model;
...
const createColumns = (model, sqlite) => {
    const columns = [];
    const { attributes } = model;

    setTimestamps(model, sqlite);
    ...
    for (const [attributeName, attributeConfig] of Object.entries(attributes)) {
        ...
        for (const key of Object.keys(attributeConfig)) {
            columnDefinition +=
                rules[key]?.(attributeConfig, attributeName) ?? '';
        }
...
const createAssociationConstraints = model => {
    ...
    //     const { foreignKey, target, referenceKey, onDelete, onUpdate } =
    //         element;
...
const generateCreate = (model, sqlite) => {
    const columns = createColumns(model, sqlite);
